...
let StoresService = class StoresService {
    ...
    async createOrUpdate(ownerId, storeData) {
        ...
        if (store) {
            ...
            store = this.storesRepository.create({ ...storeData, owner: { id: ownerId } });
        }
    ...
    async findByOwner(ownerId) {
        ...
        if (!store)
            throw new common_1.NotFoundException('Tienda no encontrada para este usuario');
        return store;
    }
    async findAll() {
    ...
    async findOnePublic(id) {
        ...
        if (!store)
            throw new common_1.NotFoundException('Tienda no encontrada o no aprobada');
        return store;
    }
};
